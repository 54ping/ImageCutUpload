        ...
        return (
            <div>
                <ImgCrop {...props}>
                    ...
                    >
                        {fileList.length >= 3 ? null : (<Button>添加图片</Button>)}　{/* {通过三木运算符判断文件列表fileList的长度来决定上传图片的数量，达到控制图片数量的功能} */}                    </Upload>
                </ImgCrop>{/* {图片预览弹出框：可以实时查看上传的图片} */}                <Modal visible={previewVisible} footer={null} onCancel={this.handleCancel} title='Image-Show'>
